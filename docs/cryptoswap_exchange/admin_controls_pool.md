...
## Pool Ownership (#insert link)
Pools created through the factory are "owned" by the factory `admin` (DAO). Ownership can only be changed within the factory contract via `commit_transfer_ownership` and `accept_transfer_ownership`. See [here](insert link here)
