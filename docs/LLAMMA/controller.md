...
!!! description "`controller.health_calculator(user: address, d_collateral: int256, d_debt: int256, full: bool, N: uint256 = 0) -> int256:`"
    ...
    | `d_collateral` |  `int256` | Change in collateral amount (signed)|
    | `d_debt` |  `int256` | Change in debt amount (signed) |
    | `full` |  `bool` | Weather its a 'full' health or not |
...
!!! description "`controller.health(user: address, full: bool = False) -> int256:`"
    ...
    Getter for the position health normalized to 1e18 for `user`.

    Returns: **health** (`int256`).

    | Input      | Type   | Description |
    | ----------- | -------| ----|
    | `user` |  `address` |  Address of the user to calculate health for |
...
!!! description "`controller.user_prices(user: address) -> uint256[2]:`"
    ...
    Getter for the lowest price of the lower band and highest price of the upper band the user has deposited in the AMM.
...
!!! description "`controller.set_monetary_policy(monetary_policy: address):"
    ...
    Function to set the monetary policy contract.
    ...
    !!!note
        Can only be called by the factory admin.

    === "Example"
        ```shell
        >>> controller.set_monetary_policy_admin("todo")
        todo
        ```
...
!!! description "`controller.create_loan(collateral: uint256, debt: uint256, N: uint256):`"
    ...
    !!!note
        `N` can range in between `MIN_TICKS` and `MAX_TICKS`. A loan can not be created if there already is an existing one.

    ??? quote "Source code"

        === "Controller.vy"

            ```python hl_lines="1 2 5 27 34 35 40 48"
...
!!! description "`controller.create_loan_extended(collateral: uint256, debt: uint256, N: uint256, callbacker: address, callback_args: DynArray[uint256,5]):`"
    ...
    Function to create a loan but pass stablecoin to a callback first so that it can build leverage.
...
!!! description "`controller.repay(_d_debt: uint256, _for: address = msg.sender, max_active_band: int256 = 2**255-1, use_eth: bool = True):`"
    ...
    Function to partially or fully repay debt.
    ...
    !!!note
        If `_d_debt` is higher than the current debt it will do a full repayment.
...
!!! description "`PoolProxy.collect_fees():`"
    ...
    Function to collect the fees charged as interest.

    !!!note
        Fees are sent to the `fee_receiver` address set in the Facotry.vy contract.
